"""Czech Extraliga schedule/standings/results scraper.

New league, explicit request 2026-09-22 (same session as the Swiss NLA
build-out). Direct port of fetch_liiga.py/fetch_shl.py/fetch_nla.py --
confirmed live that Flashscore's Czech Republic "Extraliga" pages use
the identical table markup (same .table__cell--value/--over/--under/
--score/--points classes, same /team/{slug}/{id}/ href convention, 14
real teams parsed on the first check) -- see fetch_liiga.py's own
docstring for the full design rationale (team-ID-as-key instead of a
hand-typed abbreviation table, no per-game odds yet, G/M as a real
per-team scoring-environment signal separate from GF/GA).

URLs confirmed live via the interactive browser (Flashscore's own site
navigation, not user-supplied this time):
  - fixtures:            /hockey/czech-republic/extraliga/fixtures/
  - results:             /hockey/czech-republic/extraliga/results/
  - standings (current): /hockey/czech-republic/extraliga/standings/WOgC3lWQ/standings/overall/
  - standings (25-26):   /hockey/czech-republic/extraliga-2025-2026/standings/K0tmQWEr/standings/overall/
...
PRIOR_STANDINGS_URL = f"{BASE}/extraliga-2025-2026/standings/K0tmQWEr/standings/overall/"
FIXTURES_URL = f"{BASE}/extraliga/fixtures/"
RESULTS_URL = f"{BASE}/extraliga/results/"
CURRENT_OU_URL = f"{BASE}/extraliga/standings/WOgC3lWQ/over_under/overall/5.5/"
PRIOR_OU_URL = f"{BASE}/extraliga-2025-2026/standings/K0tmQWEr/over_under/overall/5.5/"
...
def log(msg: str) -> None:
    print(f"[fetch_extraliga] {msg}", file=sys.stderr)
...
def _date_txt_to_iso(date_txt: str, season_start_year: int) -> str | None:
    """Extraliga's season also spans two calendar years (Sep-Apr, same as
    Liiga/SHL/NLA) -- July-Dec dates belong to season_start_year, Jan-Jun
    to season_start_year+1."""
